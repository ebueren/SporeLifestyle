...
```{r}
d.vir <- d.amg %>% group_by(phage, spor_host, life_bool, Taxonomy, Completeness, Host, Lifestyle, Phage_source) %>% summarise(n=n())
...
ggsave(here("refseq/out-dir/rel_lifestyle_spor_enrich.png"),
       height =3 ,width = 6)
...
labels <- 
  ...
  select(gene_id,gene) %>% 
  distinct() %>% 
  filter(gene_id %in% all.pv.adj$gene_id[d_enrich$significant]) %>% 
...
ggsave(here("refseq/out-dir/refseq_lifestyle_spor_enrich.png"),
       height =4 ,width = 6)
p
