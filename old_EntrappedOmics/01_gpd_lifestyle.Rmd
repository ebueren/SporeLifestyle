...
ps <- read.delim2(here(raw_dir, "phagescope_metadata.tsv" )) 
ps <- ps[,-c(7)]
...
all <- merge(all, ps, by.x="phage", by.y="Phage_ID", all.x=TRUE, all.y=FALSE) 


all <- merge(all, dan.tax, by.x="phage", by.y="ID", all.x=TRUE, all.y=FALSE)
#all$life_bool <- ifelse(all$Lifestyle=="temperate", TRUE, FALSE)
...
#all <- subset(all, all$Completeness=="Medium-quality" | all$Completeness=="High-quality" | all$Completeness=="Complete")

```

```{r}
# add host sporulation to amg
d.amg <- all %>% rename(spor_host = f_spor, gene_id = gene_name, gene_description = product, Lifestyle = predicted_label )
...
p <-
 ...
 ggsave(here("ivigs/IVIGS_lifestyle_spor_enrich.png"), height =4 ,width = 6)
p
...
rel <- d.amg.sig %>%
  ...
  facet_wrap(~ enriched_facet, scales = "free_x", ncol = 2) + scale_fill_manual(labels = c ("Lytic", "Temperate"), values=pal2.flip) +theme(axis.text.x = element_text(angle = 25, hjust = 1)) + xlab("Enriched Gene") + ggtitle("GPD/GVD") #+ coord_flip()

rel +  theme(legend.position="none")


 ggsave(here("ivigs/rel_refseq_lifestyle_spor_enrich.png"), height =4 ,width = 6) 
...
rel <- d.amg.sig %>%
  ...
  scale_color_hue(labels = c("Virulent", "Temperate")) +
  facet_wrap(~ enriched_facet, scales = "free_y", ncol = 2) + coord_flip()
