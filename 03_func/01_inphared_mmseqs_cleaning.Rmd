...
enrich <- read.csv(here(meta_dir, "viral_sporGenes.csv" )) 
hit <- enrich[,c(1,2)]
...
### spor_genes ALL 
master <- read.csv(here(meta_dir, "UniprotSporulationList_Master.csv"), row.names=1) 

mmseqs <- read.csv(here(raw_dir, "top5_inph_uniprot_results.m8.csv"))
...
metacerb.phrog <- read.delim2(here(raw_dir, "annotation_summary_PHROG.tsv" )) ##d_amg
...
metacerb.cog <- read.delim(here(raw_dir, "metacerb_annotation_summary_COG.tsv" )) ##d_amg
