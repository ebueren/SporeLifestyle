...
cd.clust <- read.csv(here("data/inphared_db/0a_clusters/nested/14Apr2025_0A_nested.csv"), row.names = 1)

### NEED TO MERGE IN CLUSTERS IF YOU WANT TO USE THEM!!! DUMMY!
cd.clust.key <- merge(all.hits, cd.clust, by.x="Box_Contig", by.y="Sequence_ID", all.x=TRUE, all.y=FALSE)

cd.clust.key <- na.omit(cd.clust.key)
cd.clust.key <- select(cd.clust.key, boxCluster, Cluster99, Cluster95, Cluster90, Cluster85, Cluster80)
cd.clust.key <- unique(cd.clust.key)

#duplicate_rows <- cd.clust.key[duplicated(cd.clust.key$boxCluster), ]


#unique(all.hits)
all.hits <- merge(all.hits, cd.clust.key, by="boxCluster", all.x=TRUE, all.y=FALSE)
