...
all.hits <- merge(all.hits, clust90, by.x="", by.y="", all.x=TRUE, all.y=TRUE)
...
cluster.bacil.all <- all.hits.bacil %>%
  group_by(boxCluster, phage) %>%
  summarise(count = n(), .groups = "drop") %>%
  pivot_wider(names_from = phage, values_from = count, values_fill = 0) %>%
  column_to_rownames("boxCluster") %>%
...
#bray_dist.phy <- vegdist(cluster.bacil.target, method = "jaccard")
bray_dist.phy <- vegdist(cluster.bacil.all, method = "jaccard")
# 2. Run NMDS
...
meta <- select(all.hits.bacil, phage, sporulation, lifestyle, Host, phage_type, newgtdb_Phylum, gtdb_f, newgtdb_Order, newgtdb_Class)
