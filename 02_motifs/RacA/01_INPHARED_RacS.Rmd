...
### perfect RacA
RacA <- read.delim2(here("02_motifs/RacA/data/02_out_01_indiv_hmm1_RacS_perf.tsv"))

RacA$q.value<- as.numeric(RacA$q.value)
RacA$p.value<- as.numeric(RacA$p.value)
RacA$score<- as.numeric(RacA$score)
...
## redrock (actino phage with ParABS) GU339467, uses RacA sites but of a different type than sporulation RacA

## combine phage metadata with RacA hits. Label any phage that had no RacA hits with "no_hit" in metadata
...
all <- merge(inph, RacA, by.x="Accession", by.y="sequence_name", all.x=TRUE, all.y=FALSE)
all$motif_id[is.na(all$motif_id)] <- "no_hit"

### create binary hit column of 1 for RacA hit, 0 if no hit
all$hit <- ifelse(all$motif_id=="RacA_BSub", 1, 0)
...
results <- lapply(thresholds, function(th) {
  all %>%
    ...
    summarise(
      pos.RacA.hit = max(ifelse(!is.na(q.value) & q.value <= th, 1, 0)),
      .groups = "drop"
    ) %>%
    group_by(host_phage_spor) %>%
    summarise(
      Phage_has_RacA = sum(pos.RacA.hit),
      total.phage = n(),
      .groups = "drop"
    ) %>%
    mutate(
      RacA.pos.perc = Phage_has_RacA / total.phage * 100,
      threshold = th
    )
}) %>% bind_rows()
...
# Plot
p<- ggplot(results.fig, aes(x = threshold, y = RacA.pos.perc, color = bac.host, linetype = lifestyle)) +
  geom_line(size = 1.2) +
  geom_point() +
  scale_x_log10() +  # log scale for q-values
  labs(
    x = "False Positive (q-value) Threshold",
    y = "% Phages with 1 or more RacA Binding Site"
...
ggsave(here("02_motifs/RacA/QValueTresh1e4_line.png"),width=7, height=7)
```
...
#RacA.qual$hit <- ifelse(RacA.qual$q.value<1e-4, 1, 0)
RacA.qual$hit <- ifelse(RacA.qual$q.value<1e-4, 1, 0)

## create binary list of phages w/ and w/out RacA hits
RacA.pos <- RacA.qual %>% group_by(Accession, host_phage_spor) %>%
  summarise(total.RacA.hits = sum(hit), pos.RacA.hit = max(hit), .groups = "drop") #%>% 
RacA.pos %>% count(host_phage_spor)


write.csv(RacA.pos, here("02_motifs/RacA/RacApos_1e4.csv"))
## create binary list of phages w/ and w/out RacA hits
RacA.sanity <- RacA.qual %>% group_by(Accession, host_phage_spor, Host, Description, Lowest.Taxa, Genus, Family, Genome.Length..bp., molGC....) %>%
  summarise(total.RacA.hits = sum(hit), pos.RacA.hit = max(hit), .groups = "drop") #%>% 

only.pos <- subset(RacA.sanity, RacA.sanity$total.RacA.hits>0)


RacA.pos %>% count(host_phage_spor)

RacA.summary <- RacA.pos %>% 
  group_by(host_phage_spor) %>% 
  summarise(
    total_RacA_hits = sum(total.RacA.hits, na.rm = TRUE),
    Phage_has_RacA = sum(pos.RacA.hit, na.rm = TRUE),
    total.phage = n(),
    .groups = "drop"
  ) %>% 
  mutate(RacA.pos.perc = Phage_has_RacA / total.phage * 100)


RacA.bin <- RacA.pos[,c(1,2,4)]


```

#### RacA positional analysis
```{r}


RacA.hits <- subset(RacA.qual, RacA.qual$hit==1)

## find center phage genome (whole genome /2)
RacA.hits$seq.mdpt <- as.numeric(RacA.hits$Genome.Length..bp.)/2

## find center of motif 
RacA.hits$motif.mdpt <- (RacA.hits$stop + RacA.hits$start )/ 2

### subtract midpoint motif from sequence midpoint to see how far away they are
RacA.hits <- RacA.hits %>%
  mutate(mdpt.align = (motif.mdpt - seq.mdpt))

RacA.hits$mdpt.align.kbp <- RacA.hits$mdpt.align/1000

#### TO GET RELATIVE motif alignmen

# Relative position as fraction of genome length
# (-0.5 = start, 0 = center, +0.5 = end)
RacA.hits$rel.mdpt <- (RacA.hits$motif.mdpt - RacA.hits$seq.mdpt) / RacA.hits$Genome.Length..bp.



# Relative position from genome start (0 to 1)
RacA.hits$rel.frac <- RacA.hits$motif.mdpt / RacA.hits$Genome.Length..bp.

# Optionally convert to percentage
RacA.hits$rel.percent <- RacA.hits$rel.frac * 100

##Set specific order for bacterial hosts to appear on graphs
RacA.hits$nice <- factor(RacA.hits$nice, levels = c('Lytic Phages of Sporulating Bacillota', 'Temperate Phages of Sporulating Bacillota', 'Lytic Phages of Nonsporulating Bacillota', 'Temperate Phages of Nonsporulating Bacillota', 'Lytic Phages of Non-Bacillota', 'Temperate Phages of Non-Bacillota' ),ordered = TRUE)


ggplot(RacA.hits, aes(x = mdpt.align.kbp, fill = nice)) +
  ...
  facet_wrap(~ nice, ncol = 2)+ ggtitle("Absolute Position of RacA Motif on Phage Genomes") + theme(legend.position="none")+ scale_fill_manual(values = pal7)


ggsave(here("02_motifs/RacA/figs/AbsoluteRacAposition_1e4.png"), width=7, height=7)

ggplot(RacA.hits, aes(x = rel.mdpt, fill = nice)) +
  ...
  facet_wrap(~ nice, ncol = 2) + ggtitle("Relative Position of RacA Motif on Phage Genomes")+ theme(legend.position="none") + scale_fill_manual(values = pal7)


ggsave(here("02_motifs/RacA/figs/RelativeRacAposition_1e4.png"), width=7, height=7)
```
### Enrichment
```{r}
# -----------------------------
...
n_counts <- df_all %>%
  group_by(host_phage_spor) %>%
  summarise(
    positive = sum(pos.RacA.hit == 1, na.rm = TRUE),
    total    = n(),
    .groups = "drop"
  ) %>%
  mutate(
...
vp <- ggplot(df_all, aes(x = host_phage_spor, y = pos.RacA.hit, color = host_phage_spor)) +
  geom_point(
  ...
  xlab("") + ylab("Presence of RacA in phage genomes") +
  geom_text(
    data = n_counts,
...
pairwise_res_subset <- pairwise_res_subset %>%
  ...
  mutate(y.position = max(df_all$pos.RacA.hit, na.rm = TRUE) + 
...
ggsave(here("02_motifs/RacA/figs/RacA_Enrichment_1e4.png"), width=7, height=7)
...
n_counts <- df_all %>%
  group_by(host_phage_spor) %>%
  summarise(
    positive = sum(total.RacA.hits > 0, na.rm = TRUE),
    total    = n(),
    .groups = "drop"
  ) %>%
  mutate(
...
# -----------------------------
kruskal_res <- kruskal.test(total.RacA.hits ~ host_phage_spor, data = df_all)
#print(kruskal_res)
...
y_start <- max(df_all$total.RacA.hits, na.rm = TRUE) + offset

# Compute raw p-values
pairwise_res <- lapply(seq_along(comparisons_list), function(i){
  comp <- comparisons_list[[i]]
  x <- df_all$total.RacA.hits[df_all$host_phage_spor == comp[1]]
  y <- df_all$total.RacA.hits[df_all$host_phage_spor == comp[2]]
  wt <- wilcox.test(x, y)
  data.frame(
    group1 = comp[1],
    group2 = comp[2],
...
# --- Stats ---
# Kruskal–Wallis
kw_res <- kruskal_test(df_all, total.RacA.hits ~ host_phage_spor)

# Pairwise Wilcoxon with BH correction
wilc_test <- df_all %>%
  pairwise_wilcox_test(total.RacA.hits ~ host_phage_spor,
                       p.adjust.method = "BH") #%>%
...
max_y <- max(df_all$total.RacA.hits, na.rm = TRUE)
step <- .85
wilc_test <- wilc_test %>%
...
vp <- ggplot(df_all, aes(x = host_phage_spor,
  ...
  #expand_limits(y = max(df_all$total.RacA.hits, na.rm = TRUE) * 4) +
  coord_cartesian(clip = "off") +
  theme(legend.position = "none",
        plot.margin = margin(10, 10, 30, 10)) +
  xlab("") + scale_y_continuous(limits=c(0,12.5,2),  breaks=c(0,2,4,6,8))
#vp #+ expand_limits(y = c(-1, max(df_all$total.RacA.hits) * 1.2))
...
ggsave(here("02_motifs/RacA/figs/RacA_Hits_Kruskal_1e4.png"), width=7, height=7)
```
...
analyze_enrichment <- function(df, ref_treatment = "Enriched") {
  ...
  model <- glm(pos.RacA.hit ~ host_phage_spor, family = binomial, data = df)
...
library(emmeans)
res <- analyze_enrichment(RacA.bin, ref_treatment = "Bacillota_Lytic_Spor")
...
tplot <- ggplot(fig.meta, aes(x = host_phage_spor, y = prob, color = lifestyle)) +
  ...
  ylim(0, max(pairs_df$y_pos + 0.02)) +  # extend y-axis to fit top brackets
   ylab("Probability of 1 or more RacA \n binding sites in phage genome") +
  xlab("") +
