...
ParS <- read.delim2(here("02_motifs/ParS/data/GPDGVD_02_out_00_indiv_hmm0_ParS.tsv"))
...
gpdgvd$nice <- ifelse(gpdgvd$sporulation=="Spor", "Sporulating Host", "Nonsporulating Host")

meta.cats <- unique(select(gpdgvd, host_phage_spor, bac.host, lifestyle, nice, sporulation))



#gpdgvd <- select(gpdgvd, ID, Host, contig_length, gtdb_f, f_spor, newgtdb_Phylum,  host_phage_spor, phage_type, sporulation, lifestyle, nice)
all <- merge(gpdgvd, ParS, by.x="ID", by.y="sequence_name", all.x=TRUE, all.y=FALSE)
...
all <- subset(all, all$newgtdb_Phylum!="unknown")

#all <- subset(all, all$checkv_quality=="Complete" | all$checkv_quality=="High-quality")

all <- subset(all, all$checkv_quality!="Low-quality")
```
...
ParS.pos <- ParS.qual %>% group_by(ID, sporulation) %>%
  summarise(total.ParS.hits = sum(hit), pos.ParS.hit = max(hit), .groups = "drop") #%>% 
ParS.pos %>% count(sporulation)

ParS.sanity <- ParS.qual %>% group_by(ID, host_phage_spor, gtdb_f, newgtdb_Class, newgtdb_Order) %>%
  summarise(total.ParS.hits = sum(hit), pos.ParS.hit = max(hit), .groups = "drop") #%>% 
...
ParS.hits$seq.mdpt <- as.numeric(ParS.hits$contig_length)/2
...
ParS.hits$rel.mdpt <- (ParS.hits$motif.mdpt - ParS.hits$seq.mdpt) / ParS.hits$contig_length



# Relative position from genome start (0 to 1)
ParS.hits$rel.frac <- ParS.hits$motif.mdpt / ParS.hits$contig_length
...
##### NON BACILLOTA now its own category
...
## create binary list of phages w/ and w/out ParS hits
ParS.pos <- ParS.qual %>% group_by(ID, bac.host) %>%
  summarise(total.ParS.hits = sum(hit), pos.ParS.hit = max(hit), .groups = "drop") #%>% 
ParS.pos %>% count(bac.host)

## create binary list of phages w/ and w/out ParS hits

ParS.sanity <- ParS.qual %>% group_by(ID, host_phage_spor, gtdb_f, newgtdb_Class, newgtdb_Order, bac.host) %>%
  summarise(total.ParS.hits = sum(hit), pos.ParS.hit = max(hit), .groups = "drop") #%>% 
...
ggplot(ParS.hits, aes(x = mdpt.align.kbp, fill = bac.host)) +
  geom_histogram(binwidth = 5, position = "dodge") +
  geom_vline(xintercept = 0, linetype = "dotted") +
  scale_x_continuous(breaks = seq(-90, 90, by = 30)) +
  labs(x = "Motif distance (kb) from center of phage genome", y = "Motif count") +
  facet_wrap(~ bac.host, ncol = 2) + theme(legend.position="none")+ scale_fill_manual(values = pal.shuk)+ggtitle("")#+ ggtitle("Absolute Position of ParS Motif on Phage Genomes") +


##ggsave(here("02_motifs/ParS/figs/AbsoluteParSposition_1e4_pooled.png"), width=8.5, height=7)

ggplot(ParS.hits, aes(x = rel.mdpt, fill = bac.host)) +
  geom_histogram(binwidth = 0.05, position = "dodge") +
  geom_vline(xintercept = 0, linetype = "dotted") +
  scale_x_continuous(breaks = seq(-0.5, 0.5, by = 0.25)) +
  labs(x = "Motif position relative to center of phage genome", y = "Motif count") +
  facet_wrap(~ bac.host, ncol = 2) + theme(legend.position="none") + scale_fill_manual(values = pal.shuk) +ggtitle("")#+ ggtitle("Relative Position of ParS Motif on Phage Genomes")+
...
selected_pairs <- list(
  c("Sporulating Bacillota", "Nonsporulating Bacillota"),
  c("Sporulating Bacillota", "Non-Bacillota"),
  c("Nonsporulating Bacillota", "Non-Bacillota")
)
...
df_all$bac.host <- factor(df_all$bac.host, levels = c("Sporulating Bacillota","Nonsporulating Bacillota", "Non-Bacillota"), ordered = TRUE)


vp <- ggplot(df_all, aes(x = bac.host, y = pos.ParS.hit, color = bac.host)) +
  geom_point(
    aes(fill = bac.host),
    position = pd,
    size = 2,
  ...
  geom_text(
    data = n_counts,
    aes(x = bac.host, y = -0.3, label = paste0("n=", label)),
    inherit.aes = FALSE,
    size = 4
  )
...
# Show plot
vp_fisher+ scale_fill_manual(values = pal.shuk)+ theme(legend.position="none")

#+ scale_x_discrete(labels = c('Spor'= 'Sporulating Host', 'NonSpor' = 'Nonsporulating Host')) 
...
# -----------------------------
kruskal_res <- kruskal.test(total.ParS.hits ~ bac.host, data = df_all)
#print(kruskal_res)

# -----------------------------
# Define comparisons
comparisons_list <- combn(unique(df_all$bac.host), 2, simplify = FALSE)
...
pairwise_res <- lapply(seq_along(comparisons_list), function(i){
  comp <- comparisons_list[[i]]
  x <- df_all$total.ParS.hits[df_all$bac.host == comp[1]]
  y <- df_all$total.ParS.hits[df_all$bac.host == comp[2]]
  wt <- wilcox.test(x, y)
  data.frame(
    group1 = comp[1],
    group2 = comp[2],
...
# -----------------------------
levels_order <- c("Sporulating Bacillota","Nonsporulating Bacillota", "Non-Bacillota")
...
df_all <- ParS.pos %>%
  mutate(host_phage_spor = factor(bac.host, levels = levels_order))


df_all$bac.host <- factor(df_all$bac.host, levels = levels_order, ordered = TRUE)


df_violin <- df_all %>%
  filter(total.ParS.hits > 0)
df_violin$bac.host <- factor(df_violin$bac.host, levels = levels_order, ordered = TRUE)
...
# --- Stats ---
# Kruskal–Wallis
kw_res <- kruskal_test(df_all, total.ParS.hits ~ bac.host)

# Pairwise Wilcoxon with BH correction
wilc_test <- df_all %>%
  pairwise_wilcox_test(total.ParS.hits ~ bac.host,
                       p.adjust.method = "BH") #%>%
...
vp <- ggplot(df_all, aes(x = bac.host,
                         y = total.ParS.hits,
                         color = bac.host)) +
  # Violin for positives
  geom_violin(
  ...
  theme(legend.position = "none",
        plot.margin = margin(10, 10, 10, 10)) +
  xlab("") + scale_y_continuous(limits=c(0,10,2),  breaks=c(0,2,4,6,8)) + ylab("Number of ParS Hits per Genome")
#vp #+ expand_limits(y = c(-1, max(df_all$total.ParS.hits) * 1.2))
