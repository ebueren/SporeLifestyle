...
gpdgvd$bac.host <- ifelse(gpdgvd$newgtdb_Phylum=="Bacillota", gpdgvd$bac.host, "Other Phyla")
gpdgvd$bac.host.simple <- gpdgvd$bac.host
gpdgvd$bac.host <- ifelse(gpdgvd$newgtdb_Phylum=="Actinomycetota", "Actinomycetota",gpdgvd$bac.host )
gpdgvd$bac.host <- ifelse(gpdgvd$newgtdb_Phylum=="Pseudomonadota", "Pseudomonadota",gpdgvd$bac.host)
...
meta.cats <- unique(select(gpdgvd, host_phage_spor, bac.host, lifestyle, nice, sporulation, bac.host.simple))
...
# Plot
p<- ggplot(results.fig, aes(x = threshold, y = ParS.pos.perc, color = bac.host.simple, linetype = lifestyle)) +
  geom_line(size = 1.2) +
  geom_point() +
...
####ggsave(here("02_motifs/ParS/INPH_QValueTresh1e4_line_phyla.png"),width=10, height=7)
```
...
## create binary list of phages w/ and w/out ParS hits
ParS.sanity <- ParS.qual %>%  group_by(ID, host_phage_spor, gtdb_f, newgtdb_Class, newgtdb_Order, nice, bac.host, bac.host.simple) %>%
  summarise(total.ParS.hits = sum(hit), pos.ParS.hit = max(hit), .groups = "drop") #%>% 
...
selected_pairs <- list(
  ...
  c("Sporulating Bacillota", "Actinomycetota"),
  c("Sporulating Bacillota", "Pseudomonadota"),
  c("Sporulating Bacillota", "Other Phyla"),
  c("Actinomycetota", "Other Phyla"),
  c("Pseudomonadota", "Other Phyla")
)
...
# -----------------------------
# Factor order
levels_order <- c('Sporulating Bacillota', 'Nonsporulating Bacillota', 'Actinomycetota', 'Pseudomonadota', 'Other Phyla')
...
vp <- ggplot(df_all, aes(x = bac.host, y = pos.ParS.hit, color = bac.host)) +
  ...
  scale_x_discrete(limits = levels_order, drop = FALSE) +
...
#ggsave(here("02_motifs/ParS/figs/GPDGVD_ParS_Enrichment_1e4_phyla.png"), width=12, height=7)
...
wilc_test <- subset(wilc_test, wilc_test$p.adj.signif=="ns")
...
vp <- ggplot(df_all, aes(x = bac.host,
  ...
  theme(legend.position = "none",
        plot.margin = margin(10, 10, 30, 10)) +
  xlab("") + scale_y_continuous(limits=c(0,8,2),  breaks=c(0,2,4,6,8))
#vp #+ expand_limits(y = c(-1, max(df_all$total.ParS.hits) * 1.2))


vp.krusk <- vp #+ scale_fill_manual(values = pal7)


vp.krusk

###ggsave(here("02_motifs/ParS/figs/INPH_ParS_Hits_Kruskal_1e4_phyla.png"), width=12, height=7)
```
