...
inph$bac.host <- ifelse(inph$newgtdb_Phylum=="Bacillota", inph$bac.host, "Other Phyla")
inph$bac.host.simple <- inph$bac.host
inph$bac.host <- ifelse(inph$newgtdb_Phylum=="Actinomycetota", "Actinomycetota",inph$bac.host )
inph$bac.host <- ifelse(inph$newgtdb_Phylum=="Pseudomonadota", "Pseudomonadota",inph$bac.host)

inph <- unite(inph, nice, c("lifestyle", "bac.host"), sep=" Phages of ", remove = FALSE )


meta.cats <- unique(select(inph, host_phage_spor, bac.host, lifestyle, nice, sporulation, bac.host.simple))
...
all <- subset(all, all$bac.host.simple=="Other Phyla")
```
...
ParS.pos <- ParS.qual %>% group_by(Accession, nice) %>%
...
## create binary list of phages w/ and w/out ParS hits
ParS.sanity <- ParS.qual %>% group_by(Accession, host_phage_spor, Host, Description, Lowest.Taxa, Genus, Family, Genome.Length..bp., molGC...., bac.host, nice) %>%
  summarise(total.ParS.hits = sum(hit), pos.ParS.hit = max(hit), .groups = "drop") #%>% 
...
selected_pairs <- list(
  c("Lytic Phages of Actinomycetota", "Temperate Phages of Actinomycetota"),
  c("Lytic Phages of Pseudomonadota", "Temperate Phages of Pseudomonadota"),
  c("Lytic Phages of Other Phyla", "Temperate Phages of Other Phyla"))
...
# -----------------------------
# Factor order
levels_order <- c('Lytic Phages of Actinomycetota', 'Temperate Phages of Actinomycetota', 'Lytic Phages of Pseudomonadota', 'Temperate Phages of Pseudomonadota', 'Lytic Phages of Other Phyla', 'Temperate Phages of Other Phyla')
...
vp <- ggplot(df_all, aes(x = nice, y = pos.ParS.hit, color = nice)) +
  geom_point(
    aes(fill = nice),
    position = pd,
    size = 2,
  ...
  scale_x_discrete(limits = levels_order, drop = FALSE, labels = c(
      'Lytic Phages of Actinomycetota'    = 'Lytic \n Actinomycetota',
      'Temperate Phages of Actinomycetota'     = 'Temperate \n Actinomycetota',
      'Lytic Phages of Pseudomonadota' = 'Lytic \n Pseudomonadota',
      'Temperate Phages of Pseudomonadota'  = 'Temperate \n Pseudomonadota',
      'Lytic Phages of Other Phyla' = 'Lytic \n Other Phyla',
      'Temperate Phages of Other Phyla'  = 'Temperate \n Other Phyla'
    ))  +
  ...
  geom_text(
    data = n_counts,
    aes(x = nice, y = -0.3, label = paste0("n=", label)),
    inherit.aes = FALSE,
    size = 4
  )
...
##ggsave(here("02_motifs/ParS/figs/INPHARED_ParS_Enrichment_1e4_phyla_ACINETO.png"), width=12, height=7)
...
# -----------------------------
kruskal_res <- kruskal.test(total.ParS.hits ~ nice, data = df_all)
#print(kruskal_res)

# -----------------------------
# Define comparisons
comparisons_list <- combn(unique(df_all$nice), 2, simplify = FALSE)
...
pairwise_res <- lapply(seq_along(comparisons_list), function(i){
  ...
  x <- df_all$total.ParS.hits[df_all$nice == comp[1]]
  y <- df_all$total.ParS.hits[df_all$nice == comp[2]]
  wt <- wilcox.test(x, y)
  data.frame(
    group1 = comp[1],
    group2 = comp[2],
...
df_all <- ParS.pos %>%
  mutate(nice = factor(nice, levels = levels_order))
...
# --- Stats ---
# Kruskal–Wallis
kw_res <- kruskal_test(df_all, total.ParS.hits ~ nice)

# Pairwise Wilcoxon with BH correction
wilc_test <- df_all %>%
  pairwise_wilcox_test(total.ParS.hits ~ nice, p.adjust.method = "BH") #%>%
...
##ggsave(here("02_motifs/ParS/figs/INPH_ParS_Hits_Kruskal_1e4_phyla_ACINETO.png"), width=12, height=7)
```
