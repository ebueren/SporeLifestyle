...
## create binary list of phages w/ and w/out ParS hits
ParS.pos <- ParS.qual %>% group_by(Accession, bac.host) %>%
  summarise(total.ParS.hits = sum(hit), pos.ParS.hit = max(hit), .groups = "drop") #%>% 
ParS.pos %>% count(bac.host)

## create binary list of phages w/ and w/out ParS hits
ParS.sanity <- ParS.qual %>% group_by(Accession, sporulation, Host, Description, Lowest.Taxa, Genus, Family, Genome.Length..bp., molGC...., bac.host, newgtdb_Phylum) %>%
  summarise(total.ParS.hits = sum(hit), pos.ParS.hit = max(hit), .groups = "drop") #%>% 
...
selected_pairs <- list(
  ...
  c("Nonsporulating Bacillota", "Actinomycetota"),
...
df_all$bac.host <- factor(df_all$bac.host, levels = c("Sporulating Bacillota","Nonsporulating Bacillota", "Actinomycetota", "Other Phyla"), ordered = TRUE)
...
levels_order <- c("Sporulating Bacillota","Nonsporulating Bacillota","Actinomycetota", "Other Phyla")
...
vp <- ggplot(df_all, aes(x = bac.host,
  ...
  theme(legend.position = "none",
        plot.margin = margin(10, 10, 10, 10)) +
  xlab("") + scale_y_continuous(limits=c(0,12,2),  breaks=c(0,2,4,6,8)) + ylab("Number of ParS Hits per Genome")
#vp #+ expand_limits(y = c(-1, max(df_all$total.ParS.hits) * 1.2))
