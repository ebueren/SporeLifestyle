...
meta.cats <- unique(select(inph, host_phage_spor, bac.host, lifestyle, nice, f_spor, sporulation))
...
# Plot
p<- ggplot(results.fig, aes(x = threshold, y = ParS.pos.perc, color = sporulation)) +
  geom_line(size = 1.2) +
  geom_point() +
...
## create binary list of phages w/ and w/out ParS hits
ParS.sanity <- ParS.qual %>% group_by(Accession, host_phage_spor, Host, Description, Lowest.Taxa, Genus, Family, Genome.Length..bp., molGC...., sporulation) %>%
  summarise(total.ParS.hits = sum(hit), pos.ParS.hit = max(hit), .groups = "drop") #%>% 
...
ggplot(ParS.hits, aes(x = rel.mdpt, fill = sporulation)) +
  ...
  facet_wrap(~ sporulation, ncol = 2) + ggtitle("Relative Position of ParS Motif on Phage Genomes")+ theme(legend.position="none")
...
fig.meta <- unique(fig.meta[,c(1:6)])
...
ggplot(fig.meta, aes(x = sporulation, y = prob, color = sporulation)) +
...
ggplot(ParS.pos, aes(x = factor(sporulation), fill = factor(sporulation), color=factor(sporulation), y = pos.ParS.hit )) +
...
#fig.meta$sporulation <- factor(
#  fig.meta$sporulation,
#  levels = c(
...
tplot <- ggplot(fig.meta, aes(x = sporulation, y = prob, color = sporulation)) +
