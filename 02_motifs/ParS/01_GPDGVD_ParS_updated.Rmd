...
#gpdgvd <- select(gpdgvd, ID, Host, Genome.Length..bp., gtdb_f, f_spor, newgtdb_Phylum,  host_phage_spor, phage_type, sporulation, lifestyle, nice)
all <- merge(gpdgvd, ParS, by.x="ID", by.y="genome", all.x=TRUE, all.y=FALSE)
...
#all <- subset(all, all$checkv_quality=="Complete" | all$checkv_quality=="High-quality" | all$checkv_quality=="Medium-quality")
...
#ggsave(here("02_motifs/ParS/GPDGVD_QValueTresh1e4_line.png"),width=10, height=7)
```
...
ParS.pos <- ParS.qual %>% group_by(ID, host_phage_spor) %>%
  summarise(total.ParS.hits = sum(hit), pos.ParS.hit = max(hit), .groups = "drop") #%>% 
ParS.pos %>% count(host_phage_spor)


##write.csv(ParS.pos, here("02_motifs/ParS/GPDGVD_ParSpos_1e4.csv"))
## create binary list of phages w/ and w/out ParS hits
...
#ggsave(here("02_motifs/ParS/figs/GPDGVD_AbsoluteParSposition_1e4.png"), width=10, height=7)
...
#ggsave(here("02_motifs/ParS/figs/GPDGVD_RelativeParSposition_1e4.png"), width=10, height=7)
```
### Enrichment
```{r}
# -----------------------------
...
#ggsave(here("02_motifs/ParS/figs/GPDGVD_ParS_Enrichment_1e4.png"), width=10, height=7)
...
#ggsave(here("02_motifs/ParS/figs/GPDGVD_ParS_Hits_Kruskal_1e4.png"), width=10, height=7)
```
