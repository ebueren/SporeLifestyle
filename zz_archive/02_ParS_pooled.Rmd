...
ggplot(ParS.hits, aes(x = mdpt.align.kbp, fill = sporulation)) +
  ...
  facet_wrap(~ sporulation, ncol = 2)+ ggtitle("Absolute Position of ParS Motif on Phage Genomes") + theme(legend.position="none")+ scale_fill_manual(values = pal7)


ggsave(here("02_motifs/ParS/figs/AbsoluteParSposition_1e4_pooled.png"), width=7, height=7)
...
ggplot(ParS.hits, aes(x = rel.mdpt, fill = nice)) +
  ...
  facet_wrap(~ nice, ncol = 2) + ggtitle("Relative Position of ParS Motif on Phage Genomes")+ theme(legend.position="none") + scale_fill_manual(values = pal.shuk)


ggsave(here("02_motifs/ParS/figs/RelativeParSposition_1e4_pooled.png"), width=7, height=7)
```
### Enrichment
```{r}
# -----------------------------
...
vp <- ggplot(df_all, aes(x = sporulation,
  ...
  xlab("") + scale_y_continuous(limits=c(0,8,2),  breaks=c(0,2,4,6,8))+ scale_x_discrete(labels = c('Spor'= 'Sporulating Host', 'NonSpor' = 'Nonsporulating Host')) + ylab("# of ParS Hits per Genome")
...
ggsave(here("02_motifs/ParS/figs/ParS_Hits_Kruskal_1e4_pooled.png"), width=10, height=7)
```
...
analyze_enrichment <- function(df, ref_treatment = "Enriched") {
  ...
  df$sporulation <- relevel(factor(df$sporulation), ref = ref_treatment)
  
  # logistic regression: motif presence ~ treatment
  model <- glm(pos.ParS.hit ~ sporulation, family = binomial, data = df)
  
  # estimated probabilities per treatment
  emm <- emmeans(model, ~ sporulation, type = "response")
...
library(emmeans)
res <- analyze_enrichment(ParS.bin, ref_treatment = "Spor")
...
prob_df$sporulation <- reorder(prob_df$sporulation, prob_df$prob)

meta.cats <- meta.cats[,c()]

#fig.meta <- merge(prob_df, meta.cats, by="sporulation", all=TRUE)
...
pairs_df <- pairs_df %>%
  mutate(
    x_num1 = as.numeric(factor(group1, levels = levels(fig.meta$sporulation))),
    x_num2 = as.numeric(factor(group2, levels = levels(fig.meta$sporulation))),
...
pairs_df <- pairs_df %>%
  ...
  mutate(
    y_base = max(
      fig.meta$asymp.UCL[fig.meta$sporulation %in% c(group1, group2)],
      na.rm = TRUE
    )
  ) %>%
  ungroup() %>%
...
#fig.meta$sporulation <- factor(fig.meta$sporulation, levels = c("Spor","NonSpor"), ordered = TRUE)
prob_df$sporulation <- factor(prob_df$sporulation, levels = c("Spor","NonSpor"), ordered = TRUE)


tplot <- ggplot(prob_df, aes(x = sporulation, y = prob, color = sporulation)) +
  ...
  theme_classic(base_size = 14) + theme(plot.margin = margin(10, 10, 10, 10)) +
  coord_cartesian(clip = "off") +theme(legend.position = "none")+ scale_x_discrete(labels = c('Spor'= 'Sporulating Host', 'NonSpor' = 'Nonsporulating Host'))


tplot+ scale_color_manual(values = pal.shuk)
