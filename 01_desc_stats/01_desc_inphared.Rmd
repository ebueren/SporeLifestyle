...
inph$cds.log <- log10(inph$Number.CDS)
...
inph.subset <- subset(inph.all, inph.all$Host=="Bacillus" | inph.all$Host=="Clostridioides" | inph.all$Host=="Staphylococcus" | inph.all$Host=="Clostridium" | inph.all$Host=="Enterococcus" | inph.all$Host== "Lactobacillus" | inph.all$Host=="Paenibacillus" | inph.all$Host=="Listeria" | inph.all$Host=="Streptococcus" )
...
inph.genes.log <- ggplot(inph.bac, aes(x = factor(lifestyle), fill = factor(lifestyle), y = cds.log)) + 
  geom_boxplot(binaxis = "y", stackdir = "center", position = "dodge") + geom_jitter(width = 0.2)  + scale_fill_manual(values=pal2) + ylab("Number of Genes") + xlab("") + ggtitle("INPHARED") + labs(fill="Lifestyle") + facet_wrap(~ newgtdb_Phylum2, ncol = 2)


##lifestyle vs. genome size
inph.log


### lifesytle vs. # of genes
inph.genes
...
twoway.inph.m <- aov(genome.log ~ lifestyle * sporulation, data = inph.bac)
...
gvC.log <- ggplot(inph.all, aes(Genome.Length..bp., cds.log, fill=phage_type, colour = phage_type, shape = f_spor)) +
  geom_point() +
  geom_smooth(method="lm")+ # Add regression line
 stat_regline_equation(aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~~")),
                        label.x.npc = "left", label.y.npc = "top" )# Add equation#+ ylim(0, 25) #+ xlim(0, 400000)
...
model <- lm(genome.log ~ cds.log, data = inph.all)
summary(model)

plot(inph.all$genome.log, inph.all$cds.log)
